...
import { featureParagraphs, normalizeUrl, period, projectImages } from "../../utils/guest";
...
export default function ProjectDetail() {
  ...
  if (projectsLoading || projectsError) return <>
    <PageMeta title="Proyek — Portofolio" />
    <Link to={backTo} state={{ restore: true }} className="back-link"><Icon name="left" size={18} />Kembali ke proyek</Link>
    <ProjectState loading={projectsLoading} error={projectsError} onRetry={reloadProjects} />
  </>;
  ...
  const hasPeriod = Boolean(project.tanggal_mulai || project.tanggal_selesai);
  const hasInfo = Boolean(project.status || hasPeriod || images.length || github || demo);
  ...
  return <div className="project-detail">
    ...
    <Link to={backTo} state={{ restore: true }} className="back-link"><Icon name="left" size={18} />Kembali ke proyek</Link>
    <header className="detail-heading"><p className="eyebrow">Proyek <span>/ {String(index + 1).padStart(2, "0")}</span></p><h1>{name}</h1>{(project.status || hasPeriod) && <div className="detail-heading-meta"><Status value={project.status} />{hasPeriod && <span className="mono">{period(project)}</span>}</div>}</header>
    <div className={`detail-layout ${!hasInfo ? "detail-layout-single" : ""}`}><article className="detail-story">
