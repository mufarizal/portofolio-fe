...
export default function Collection({ certificate = false }) {
  ...
  const items = certificate ? data.sertifikats : projects;
  const title = certificate ? "Sertifikat" : "Proyek";
  const query = params.get("q") || "";
  const filter = params.get("filter") || "";
  const defaultSort = certificate ? "newest" : "editorial";
  const sort = certificate ? (params.get("sort") === "oldest" ? "oldest" : "newest") : (params.get("sort") === "name" ? "name" : "editorial");
  ...
  const update = (field, value) => {
    ...
    if (value && !(field === "sort" && value === defaultSort) && !(field === "page" && value === "1")) next.set(field, value); else next.delete(field);
  ...
  return <div className="collection-page">
    ...
    {filtered.length ? certificate ? <div className="certificate-list">{result.items.map((item) => <CertificateRow key={item.id} certificate={item} />)}</div> : <div className="project-grid">{result.items.map((item, index) => <ProjectCard key={item.id} project={item} index={(result.page - 1) * 6 + index} />)}</div> : <EmptyState title={items.length ? "Tidak ada hasil yang cocok" : certificate ? "Sertifikat belum ditambahkan" : "Belum ada project yang aktif"} message={items.length ? "Coba kata kunci lain atau hapus filter untuk melihat seluruh koleksi." : "Koleksi akan muncul di halaman ini setelah tersedia."}>{items.length > 0 && <button className="button button-secondary" onClick={clear}>Tampilkan semua</button>}</EmptyState>}
